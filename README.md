...
- **UI:** http://localhost:8001/login (или /register, /dashboard, /profile, /payments, /chat, /admin, /confirm)  
...
**Дашборд:** иконка уведомлений (конверт) рядом с кнопкой «Выйти». Если не заполнены имя, фамилия или телефон в профиле — показывается бейдж; при клике открывается панель с подсказками «Заполните имя в профиле» и т.д.
...
- OTP из `otp/preview` живёт 1 минуту и привязан к пользователю.

### Ручки

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/helper/otp/preview` | Получить OTP для операций (динамический, живёт 1 минуту) |
...
| GET | `/admin/users/{user_id}/accounts` | Счета пользователя (для пополнения) |
...
| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/auth/register` | Регистрация. Тело: `{"login": "...", "password": "..."}`. Ответ 201: объект пользователя (id, login, role, status, first_name, last_name, phone, email). Поля `first_name`, `last_name` — пустая строка `""` если не заполнены. |
| POST | `/auth/login` | Вход. Тело: `{"login": "...", "password": "..."}`. Ответ 200: `{"access_token": "...", "token_type": "bearer", "role": "ADMIN"|"CLIENT"}`. |
...
- OTP: 4 цифры, **только** из `GET /helper/otp/preview` (динамический, живёт 1 минуту).
...
Ручка возвращает фиксированный JSON. Темой, языком и уведомлениями пользователь не управляет.

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/settings` | Ответ: `theme`, `language`, `notificationsEnabled`. Требуется Bearer-токен. |
